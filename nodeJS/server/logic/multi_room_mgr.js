...
/**
 * 心跳检查 
 * 会定时检查，房间成员的心跳是否超时，超时时间由config.multi_room.heartBeatTimeout指定。
 * 当房间成员为空，也会将对应的房间删除。
 */
function onTimer() {
	var i = 0;
	while (i < rooms.length) {
		...
		while (j < rooms[i].pushers.length) {
			...
			// 超时处理
			if (timeSpan(rooms[i].pushers[j].timestamp) > config.multi_room.heartBeatTimeout) {
...
function getRoomList(cnt, startpos, withpushers) {
	...
	if (withpushers) {
		for (i in rooms) {
			if (getMemberCnt(rooms[i].roomID) <= config.multi_room.maxMembers && i >= startpos) {
				var room = {};
